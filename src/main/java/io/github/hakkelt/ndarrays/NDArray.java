...
public interface NDArray<T> extends Iterable<T> {
    ...
    /**
     * Returns a view that references this NDArray as parent, and skips all singleton dimensions included in the
     * parameter list.
     * 
     * <p>
     * View: An NDArray that references the specified region its parent array. All modifications in the parent array are
     ...
     * @return a view that references this NDArray as parent, and skips all singleton dimensions included in the
     *         parameter list
     */
    ...
    /**
     * Returns an array view referencing this NDArray as parent that gives read-write access to a specific elements of
     * the array selected by the given inverseMask.
     * 
     * The inverseMask must have the same shape as this array, and those entries are selected which has the same indices as the
     * non-zero entries in the inverseMask. In other words: All places where the inverseMask contains a zero value are skipped, and
     * all other values are copied into a new vector.
     * 
