 ...
 */
public abstract class AbstractNDArray<T, T2 extends Number> implements NDArray<T> {
    ...
    public T2 getImag(int linearIndex) {
        NDArrayUtils.boundaryCheck(linearIndex, length());
        return getImagUnchecked(NDArrayUtils.unwrap(linearIndex, length()));
    }
    ...
    public T2 getImag(int... indices) {
        NDArrayUtils.boundaryCheck(indices, shape());
        return getImagUnchecked(NDArrayUtils.unwrap(indices, shape()));
    }
    ...
    public void setImag(T2 value, int linearIndex) {
        NDArrayUtils.boundaryCheck(linearIndex, length());
        setImagUnchecked(value, NDArrayUtils.unwrap(linearIndex, length()));
    }
    ...
    public void setImag(T2 value, int... indices) {
        NDArrayUtils.boundaryCheck(indices, shape());
        setImagUnchecked(value, NDArrayUtils.unwrap(indices, shape()));
    }
    ...
    protected abstract T accumulate(BigInteger acc, NDArray<?> array, int linearIndex, AccumulateOperators operator);

    protected abstract T accumulate(BigDecimal acc, NDArray<?> array, int linearIndex, AccumulateOperators operator);
    ...
    protected abstract T accumulate(BigInteger acc, Number value, AccumulateOperators operator);

    protected abstract T accumulate(BigDecimal acc, Number value, AccumulateOperators operator);
    ...
    protected void copyTo1DArray(short[] array, int[] index) {
        for (int i = 0; i < shape[shape.length - 1]; i++) {
            index[shape.length - 1] = i;
            array[i] = ((Number) get(index)).shortValue();
        }
    }

    protected void copyTo1DArray(int[] array, int[] index) {
        for (int i = 0; i < shape[shape.length - 1]; i++) {
            index[shape.length - 1] = i;
            array[i] = ((Number) get(index)).intValue();
        }
    }

    protected void copyTo1DArray(long[] array, int[] index) {
        for (int i = 0; i < shape[shape.length - 1]; i++) {
            index[shape.length - 1] = i;
            array[i] = ((Number) get(index)).longValue();
        }
    }

    protected void copyTo1DArray(float[] array, int[] index) {
        for (int i = 0; i < shape[shape.length - 1]; i++) {
            index[shape.length - 1] = i;
            array[i] = ((Number) get(index)).floatValue();
        }
    }

    protected void copyTo1DArray(double[] array, int[] index) {
        for (int i = 0; i < shape[shape.length - 1]; i++) {
            index[shape.length - 1] = i;
            array[i] = ((Number) get(index)).doubleValue();
        }
    }
    ...
    protected void copyTo1DArray(Integer[] array, int[] index) {
        for (int i = 0; i < shape[shape.length - 1]; i++) {
            index[shape.length - 1] = i;
            array[i] = ((Number) get(index)).intValue();
        }
