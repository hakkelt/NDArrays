...
public class ViewOperations<T, T2 extends Number> {
    ...
    public static boolean isThisSlicingAnIdentityOperation(NDArray<?> me, NormalizedRange[] slicingExpressions) {
        return me.ndim() == slicingExpressions.length && Stream.of(slicingExpressions).allMatch(NormalizedRange::isAll);
    }
    ...
    public static Object[] selectedDimsToSlicingExpression(int[] shape, Set<Integer> selectedDims) {
        return IntStream.range(0, shape.length).mapToObj(i -> {
            if (selectedDims.contains(i))
                return ":";
            if (shape[i] != 1)
                throw new IllegalArgumentException(String.format(Errors.DROPDIMS_NOT_SINGLETON, i));
            return 0;
        }).toArray();
    }
    ...
    public NDArray<T2> slice(RealNDArray<T2> me, Object... slicingExpressions) {
        ...
        NormalizedRange[] normalizedRanges = NormalizedRange.normalizeRanges(expressions, me.shape());
        if (isThisSlicingAnIdentityOperation(me, normalizedRanges))
            return me;
        return new RealNDArraySliceView<>(me, normalizedRanges);
    }
    ...
    public ComplexNDArray<T2> slice(ComplexNDArray<T2> me, Object... slicingExpressions) {
        ...
        if (isThisSlicingAnIdentityOperation(me, normalizedRanges))
            return me;
        return new ComplexNDArraySliceView<>(me, normalizedRanges);
    }
    ...
    public NDArray<T2> permuteDims(RealNDArray<T2> me, int... permutation) {
        if (isThisPermutationAnIdentityOperation(permutation))
            return me;
    ...
    public ComplexNDArray<T2> permuteDims(ComplexNDArray<T2> me, int... permutation) {
        ...
        return IntStream.range(0, me.ndim()).allMatch(i -> i == view.dimsOrder[i]) ? (ComplexNDArray<T2>) view.parent
                : view;
    }

    public NDArray<T2> reshape(RealNDArray<T2> me, int... newShape) {
        if (isThisReshapingAnIdentityOperation(me.shape(), newShape))
            return me;
    ...
    public NDArray<T> dropDims(AbstractNDArray<T, T2> me, int... droppedDims) {
        ...
        if (set.isEmpty())
            throw new IllegalArgumentException(Errors.ALL_DIMS_DROPPED);
