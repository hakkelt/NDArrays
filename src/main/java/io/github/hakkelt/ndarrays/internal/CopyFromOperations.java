...
public class CopyFromOperations<T, T2 extends Number> {
    ...
    protected void flatten(AbstractNDArray<T,T2> me, BigInteger[] real, int startIndex, int dimension) {
        IntStream.range(0, real.length).parallel()
            .forEach(i -> me.setUnchecked(me.wrapValue(real[i]), startIndex + i * me.multipliers[dimension]));
    }

    protected void flatten(AbstractNDArray<T,T2> me, BigDecimal[] real, int startIndex, int dimension) {
        IntStream.range(0, real.length).parallel()
            .forEach(i -> me.setUnchecked(me.wrapValue(real[i]), startIndex + i * me.multipliers[dimension]));
    }

    protected void flatten(AbstractNDArray<T,T2> me, Complex[] real, int startIndex, int dimension) {
        IntStream.range(0, real.length).parallel()
            .forEach(i -> me.setUnchecked(me.wrapValue(real[i]), startIndex + i * me.multipliers[dimension]));
    }
    ...
    @SuppressWarnings("unchecked")
    protected void flatten(ComplexNDArray<T2> me, BigInteger[] real, BigInteger[] imag, int startIndex, int dimension) {
        AbstractNDArray<T, T2> casted = (AbstractNDArray<T, T2>) me;
        IntStream.range(0, real.length).parallel()
            .forEach(i -> {
                int linearIndex = startIndex + i * ((AbstractNDArray<T,T2>)me).multipliers[dimension];
    ...
    @SuppressWarnings("unchecked")
    protected void flatten(ComplexNDArray<T2> me, BigDecimal[] real, BigDecimal[] imag, int startIndex, int dimension) {
        AbstractNDArray<T, T2> casted = (AbstractNDArray<T, T2>) me;
        IntStream.range(0, real.length).parallel()
            .forEach(i -> {
                int linearIndex = startIndex + i * ((AbstractNDArray<T,T2>)me).multipliers[dimension];
