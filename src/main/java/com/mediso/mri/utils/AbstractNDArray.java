...
abstract class AbstractNDArray<T> extends AbstractCollection<T> implements NDArray<T> {
    ...
    @Override
    public DataSet toRS2DDataSet() {
        if (ndims() > 5)
            throw new UnsupportedOperationException(
                String.format(ERROR_RS2D_DATA_SET_TOO_HIGH_DIMENSIONAL, ndims()));
        
        int matrix1D = dims(0);
        int matrix2D = ndims() > 1 ? dims(1) : 1;
        int matrix3D = ndims() > 2 ? dims(2) : 1;
        int matrix4D = ndims() > 3 ? dims(3) : 1;
        int receiver = ndims() == 5 ? dims(4) : 1;

        DataSet dataSet = new DataSet(matrix1D, matrix2D, matrix3D, matrix4D, receiver, ModalityEnum.MRI);
        for (int j = 0; j < matrix2D; j++)
            for (int k = 0; k < matrix3D; k++)
                for (int t = 0; t < matrix4D; t++)
                    for (int c = 0; c < receiver; c++)
                        dataSet.putData(getRealPart(matrix1D, j, k, t, c), getImagPart(matrix1D, j, k, t, c), j, k, t, c);
        return dataSet;
    }

    
    @Override
    public Image toSimpleITKImage() {
        if (eltype() == Complex.class)
            throw new UnsupportedOperationException(ERROR_SIMPLE_ITK_COMPLEX_NOT_SUPPORTED);
        VectorUInt32 size = new VectorUInt32(ndims());
        for (int i = 0; i < ndims(); i++)
            size.set(i, dims[i]);
        Image image;
    ...
    public NDArray<T> copyFrom(double[] real, double[] imag) {
        if (eltype() != Complex.class)
    ...
    public NDArray<T> copyFrom(Object[] real, Object[] imag) {
        if (eltype() != Complex.class)
            throw new UnsupportedOperationException(ERROR_COPY_FROM_COMPLEX_UNSUPPORTED);
        flatten(real, imag, 0, 0);
        return this;
    ...
    @SuppressWarnings("unchecked")
    public NDArray<T> copyFrom(NDArray<?> real, NDArray<?> imag) {
        if (eltype() != Complex.class)
            throw new UnsupportedOperationException(ERROR_COPY_FROM_COMPLEX_UNSUPPORTED);
    ...
    public NDArray<T> dropDims(int... selectedDims) {
        List<Integer> set = IntStream.of(selectedDims).boxed().collect(Collectors.toList());
        return selectDims(IntStream.range(0, selectedDims.length).filter(i -> !set.contains(i)).toArray());
    }
    ...
    protected void incrementSimpleITKIndex(VectorUInt32 index, int dimension) {
        if (index.get(dimension) == dims(dimension) - 1) {
            index.set(dimension, 0);
            incrementSimpleITKIndex(index, dimension + 1);
        } else {
            index.set(dimension, index.get(dimension) + 1);
        }
    }
