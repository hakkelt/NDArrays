...
class TestBasicFloatNDArrayPermuteDims extends TestBase {
    ...
    void testNotEqual() {
        ...
        assertNotEquals(pArray, new BasicFloatNDArray(6, 10));
        assertNotEquals(pArray, new BasicFloatNDArray(4, 5, 3));
    }
    ...
    void testToString() {
        ...
        assertEquals(array.getNamePrefix() + " NDArrayView<Float>(4 × 3 × 5)", str);
    }
    ...
    void testContentToString() {
        ...
        sb.append(array.getNamePrefix() + " NDArrayView<Float>(4 × 3 × 5)" + System.lineSeparator());
        for (int i = 0; i < pArray.shape(2); i++) {
            ...
            for (int j = 0; j < pArray.shape(0); j++) {
                if (pArray.dtype() == Complex.class)
                    ...
                    sb.append(String.format(lineFormat, wrapToFloat(i * 4 + j), wrapToFloat(i * 4 + j + 20), wrapToFloat(i * 4 + j + 40)));
            }
    ...
    void testConcatenate() {
        NDArray<Float> array2 = new BasicFloatNDArray(2, 3, 5).fill(1);
        NDArray<Float> array3 = pArray.concatenate(0, array2);
        ...
        for (int i = pArray.shape(0); i < pArray.shape(0) + array2.shape(0); i++)
            for (int j = 0; j < pArray.shape(1); j++)
                for (int k = 0; k < pArray.shape(1); k++)
                    assertSpecializedEquals(wrapToFloat(1), array3.get(i, j, k));
    }

    @Test
    void testConcatenateMultiple() {
        NDArray<Float> array2 = pArray.copy().fill(1).slice(":", "1:3", ":");
        NDArray<Float> array3 = new BasicFloatNDArray(2, 5, 4).permuteDims(2, 0, 1);
        NDArray<Float> array4 = new BasicFloatNDArray(20).fill(2).reshape(4, 1, 5);
        NDArray<Float> array5 = pArray.concatenate(1, array2, array3, array4);
        int start = 0;
        ...
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                for (int k = 0; k < array2.shape(2); k++)
                    assertSpecializedEquals(wrapToFloat(1), array5.get(i, j, k));
        start = end;
        end += array3.shape(1);
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                for (int k = 0; k < array2.shape(2); k++)
                    assertSpecializedEquals(wrapToFloat(0), array5.get(i, j, k));
        start = end;
        end += array4.shape(1);
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                for (int k = 0; k < array2.shape(2); k++)
                    assertSpecializedEquals(wrapToFloat(2), array5.get(i, j, k));
    }
    ...
    void testPermuteDimsWithSlice() {
        NDArray<Float> pArray2 = slice.permuteDims(1, 0);
    ...
    void testPermuteDimsWithReshaped() {
        NDArray<Float> pArray2 = reshaped.permuteDims(1, 0);
    ...
    void testPermuteDimsWithPArray() {
        NDArray<Float> pArray2 = pArray.permuteDims(0, 2, 1);
    ...
    void testPermuteDimsWithMasked() {
        NDArray<Float> pArray2 = masked.permuteDims(0);
