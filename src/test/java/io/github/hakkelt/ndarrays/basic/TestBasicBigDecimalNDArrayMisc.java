...
class TestBasicBigDecimalNDArrayMisc extends TestBase {
    ...
    void setup() {
        ...
        array.fillUsingLinearIndices(TestBase::wrapToBigDecimal);
    ...
    void testDTypeWithArray() {
        assertEquals(BigDecimal.class, array.dtype());
    }

    @Test
    void testDTypeWithSlice() {
        assertEquals(BigDecimal.class, slice.dtype());
    }

    @Test
    void testDTypeWithReshaped() {
        assertEquals(BigDecimal.class, reshaped.dtype());
    }

    @Test
    void testDTypeWithPArray() {
        assertEquals(BigDecimal.class, pArray.dtype());
    }

    @Test
    void testDTypeWithMasked() {
        assertEquals(BigDecimal.class, masked.dtype());
    }

    @Test
    void testWrongToArrayWithArray() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> array.toArray(new BigDecimal[0][][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 4, 3), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithSlice() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> slice.toArray(new BigDecimal[0][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 3, 2), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithReshaped() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> reshaped.toArray(new BigDecimal[0][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 3, 2), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithPArray() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> pArray.toArray(new BigDecimal[0][][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 4, 3), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithMasked() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> masked.toArray(new BigDecimal[0][][]));
    ...
    void testNotEqualsWithArray() {
        ...
        assertNotEquals(array, new BasicBigDecimalNDArray(IntStream.of(array.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(array, new BasicBigDecimalNDArray(array.shape()));
    }
    ...
    void testNotEqualsWithSlice() {
        ...
        assertNotEquals(slice, new BasicBigDecimalNDArray(IntStream.of(slice.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(slice, new BasicBigDecimalNDArray(slice.shape()));
    }
    ...
    void testNotEqualsWithReshaped() {
        ...
        assertNotEquals(reshaped, new BasicBigDecimalNDArray(IntStream.of(reshaped.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(reshaped, new BasicBigDecimalNDArray(reshaped.shape()));
    }
    ...
    void testNotEqualsWithPArray() {
        ...
        assertNotEquals(pArray, new BasicBigDecimalNDArray(IntStream.of(pArray.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(pArray, new BasicBigDecimalNDArray(pArray.shape()));
    }
    ...
    void testNotEqualsWithMasked() {
        ...
        assertNotEquals(masked, new BasicBigDecimalNDArray(IntStream.of(masked.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(masked, new BasicBigDecimalNDArray(masked.shape()));
    }
    ...
    void testSliceEqualsWithSameType() {
        NDArray<BigDecimal> slice2 = array.slice(1, "1:4", "0:");
        assertEquals(slice, slice2);
        NDArray<BigDecimal> slice3 = array.copy().slice(1, "1:4", "0:");
    ...
    void testPArrayEqualsWithSameType() {
        NDArray<BigDecimal> pArray2 = array.permuteDims(0, 2, 1);
        assertEquals(pArray, pArray2);
        NDArray<BigDecimal> pArray3 = array.copy().permuteDims(0, 2, 1);
    ...
    void testMaskedEqualsWithSameType() {
        ...
        NDArray<BigDecimal> masked3 = array.copy().mask(mask);
        assertEquals(masked, masked3);
        NDArray<BigDecimal> masked4 = array.mask(value -> value.intValue() % 2 == 1);
        assertEquals(masked, masked4);
        NDArray<BigDecimal> masked5 = array.copy().mask(value -> value.intValue() % 2 == 1);
        assertEquals(masked, masked5);
        NDArray<BigDecimal> masked6 = array.maskWithLinearIndices((value, i) -> i.intValue() % 2 == 1);
        assertEquals(masked, masked6);
        NDArray<BigDecimal> masked7 = array.copy().maskWithLinearIndices((value, i) -> i.intValue() % 2 == 1);
        assertEquals(masked, masked7);
        NDArray<BigDecimal> masked8 = array.maskWithCartesianIndices((value, idx) -> value.intValue() % 2 == 1);
        assertEquals(masked, masked8);
        NDArray<BigDecimal> masked9 = array.copy().maskWithCartesianIndices((value, idx) -> value.intValue() % 2 == 1);
    ...
    void test0Norm() {
        ...
        double norm = array.stream().filter(value -> Math.abs(value.doubleValue()) != 0.).count();
        assertEquals(norm, array.norm(0));
    }

    @Test
    void test1NormWithArray() {
        double norm = array.stream().mapToDouble(value -> Math.abs(value.doubleValue())).reduce(0., (acc, item) -> acc + item);
        assertEquals(norm, array.norm(1));
    }

    @Test
    void test1NormWithSlice() {
        double norm = slice.stream().mapToDouble(value -> Math.abs(value.doubleValue())).reduce(0., (acc, item) -> acc + item);
        assertEquals(norm, slice.norm(1));
    }

    @Test
    void test1NormWithReshaped() {
        double norm = reshaped.stream().mapToDouble(value -> Math.abs(value.doubleValue())).reduce(0., (acc, item) -> acc + item);
        assertEquals(norm, reshaped.norm(1));
    }

    @Test
    void test1NormWithPArray() {
        double norm = pArray.stream().mapToDouble(value -> Math.abs(value.doubleValue())).reduce(0., (acc, item) -> acc + item);
        assertEquals(norm, pArray.norm(1));
    }

    @Test
    void test1NormWithMasked() {
        double norm = masked.stream().mapToDouble(value -> Math.abs(value.doubleValue())).reduce(0., (acc, item) -> acc + item);
    ...
    void test2NormWithArray() {
        double norm = Math
                .sqrt(array.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 2)).reduce(0., (acc, item) -> acc + item));
    ...
    void test2NormWithSlice() {
        double norm = Math
                .sqrt(slice.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 2)).reduce(0., (acc, item) -> acc + item));
    ...
    void test2NormWithReshaped() {
        double norm = Math
                .sqrt(reshaped.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 2)).reduce(0., (acc, item) -> acc + item));
    ...
    void test2NormWithPArray() {
        double norm = Math
                .sqrt(pArray.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 2)).reduce(0., (acc, item) -> acc + item));
    ...
    void test2NormWithMasked() {
        double norm = Math
                .sqrt(masked.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 2)).reduce(0., (acc, item) -> acc + item));
    ...
    void testPQuasinormWithArray() {
        double norm = Math.pow(
                array.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 0.5)).reduce(0., (acc, item) -> acc + item), 2);
    ...
    void testPQuasinormWithSlice() {
        double norm = Math.pow(
                slice.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 0.5)).reduce(0., (acc, item) -> acc + item), 2);
    ...
    void testPQuasinormWithReshaped() {
        double norm = Math.pow(
                reshaped.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 0.5)).reduce(0., (acc, item) -> acc + item), 2);
    ...
    void testPQuasinormWithPArray() {
        double norm = Math.pow(
                pArray.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 0.5)).reduce(0., (acc, item) -> acc + item), 2);
    ...
    void testPQuasinormWithMasked() {
        double norm = Math.pow(
                masked.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 0.5)).reduce(0., (acc, item) -> acc + item), 2);
    ...
    void testPNormWithArray() {
        double norm = Math.pow(
                array.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 3.5)).reduce(0., (acc, item) -> acc + item),
                1 / 3.5);
    ...
    void testPNormWithSlice() {
        double norm = Math.pow(
                slice.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 3.5)).reduce(0., (acc, item) -> acc + item),
                1 / 3.5);
    ...
    void testPNormWithReshaped() {
        double norm = Math.pow(
                reshaped.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 3.5)).reduce(0., (acc, item) -> acc + item),
                1 / 3.5);
    ...
    void testPNormWithPArray() {
        double norm = Math.pow(
                pArray.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 3.5)).reduce(0., (acc, item) -> acc + item),
                1 / 3.5);
    ...
    void testPNormWithMasked() {
        double norm = Math.pow(
                masked.stream().map(value -> Math.pow(Math.abs(value.doubleValue()), 3.5)).reduce(0., (acc, item) -> acc + item),
                1 / 3.5);
    ...
    void testInfNormWithArray() {
        double norm = (float) array.stream().mapToDouble(value -> Math.abs(value.doubleValue())).max().getAsDouble();
        assertEquals(norm, array.norm(Double.POSITIVE_INFINITY));
    }

    @Test
    void testInfNormWithSlice() {
        double norm = (float) slice.stream().mapToDouble(value -> Math.abs(value.doubleValue())).max().getAsDouble();
        assertEquals(norm, slice.norm(Double.POSITIVE_INFINITY));
    }

    @Test
    void testInfNormWithReshaped() {
        double norm = (float) reshaped.stream().mapToDouble(value -> Math.abs(value.doubleValue())).max().getAsDouble();
        assertEquals(norm, reshaped.norm(Double.POSITIVE_INFINITY));
    }

    @Test
    void testInfNormWithPArray() {
        double norm = (float) pArray.stream().mapToDouble(value -> Math.abs(value.doubleValue())).max().getAsDouble();
        assertEquals(norm, pArray.norm(Double.POSITIVE_INFINITY));
    }

    @Test
    void testInfNormWithMasked() {
        double norm = (float) masked.stream().mapToDouble(value -> Math.abs(value.doubleValue())).max().getAsDouble();
    ...
    void testFillDoubleWithArray() {
        array.fill(3);
        for (BigDecimal elem : array)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }

    @Test
    void testFillDoubleWithSlice() {
        slice.fill(3);
        for (BigDecimal elem : slice)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }
    ...
    void testFillDoubleWithMasked() {
        masked.fill(3);
        for (BigDecimal elem : masked)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }

    @Test
    void testFillTWithArray() {
        array.fill(wrapToBigDecimal(3));
        for (BigDecimal elem : array)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }
    ...
    void testFillTWithReshaped() {
        reshaped.fill(wrapToBigDecimal(3));
        for (BigDecimal elem : reshaped)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }
    ...
    void testFillTWithPArray() {
        pArray.fill(wrapToBigDecimal(3));
        for (BigDecimal elem : pArray)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }
    ...
    void testFillTWithMasked() {
        masked.fill(wrapToBigDecimal(3));
        for (BigDecimal elem : masked)
            assertSpecializedEquals(wrapToBigDecimal(3), elem);
    }
    ...
    void testContaintsWithArray() {
        assertTrue(array.contains(wrapToBigDecimal(5)));
    }

    @Test
    void testContaintsWithSlice() {
        assertTrue(slice.contains(wrapToBigDecimal(5)));
    }

    @Test
    void testContaintsWithReshaped() {
        assertTrue(reshaped.contains(wrapToBigDecimal(5)));
    }

    @Test
    void testContaintsWithPArray() {
        assertTrue(pArray.contains(wrapToBigDecimal(5)));
    }

    @Test
    void testContaintsWithMasked() {
        assertTrue(masked.contains(wrapToBigDecimal(5)));
    }
