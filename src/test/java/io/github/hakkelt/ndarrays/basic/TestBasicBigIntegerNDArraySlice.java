...
class TestBasicBigIntegerNDArraySlice extends TestBase {
    ...
    @Test
    void testSliceSlice() {
        NDArray<BigInteger> slice2 = slice.slice(":", 2);
    ...
    void testStrided() {
        ...
        NDArray<BigInteger> slice2 = array.slice(new Range(1, 5, 4), ":2:", new Range(0, 3));
    ...
    void testStridedNegativeStepSize() {
        int[] expectedshape = { 1, 3, 3 };
        NDArray<BigInteger> slice2 = array.slice(new Range(-1, -5, 0), ":-2:", "0::3");
        assertArrayEquals(expectedshape, slice2.shape());
        assertEquals(IntStream.of(expectedshape).reduce(1, (acc, value) -> acc * value), slice2.length());
        assertEquals(expectedshape[0], slice2.shape(0));
        assertEquals(expectedshape.length, slice2.ndim());
    ...
    void testToString() {
        ...
        assertEquals(array.getNamePrefix() + " NDArrayView<BigInteger>(3 × 3)", str);
    }
    ...
    void testcontentToString() {
        ...
        if (!array.dtype().equals(Complex.class)) {
            expected = new StringBuilder()
                    .append(array.getNamePrefix() + " NDArrayView<BigInteger>(3 × 3)" + System.lineSeparator())
                    .append(String.format(lineFormat, wrapToBigInteger(5), wrapToBigInteger(25), wrapToBigInteger(45)))
                    .append(String.format(lineFormat, wrapToBigInteger(9), wrapToBigInteger(29), wrapToBigInteger(49)))
                    .append(String.format(lineFormat, wrapToBigInteger(13), wrapToBigInteger(33), wrapToBigInteger(53))).toString();
        } else {
            expected = new StringBuilder()
                    .append(array.getNamePrefix() + " NDArrayView<BigInteger>(3 × 3)" + System.lineSeparator())
    ...
    void testConcatenate() {
        NDArray<BigInteger> array2 = new BasicBigIntegerNDArray(3, 5).fill(1);
        NDArray<BigInteger> array3 = slice.concatenate(1, array2);
        ...
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = slice.shape(1); j < slice.shape(1) + array2.shape(1); j++)
                assertSpecializedEquals(wrapToBigInteger(1), array3.get(i, j));
    }

    @Test
    void testConcatenateMultiple() {
        NDArray<BigInteger> array2 = array.copy().fill(1).slice(":3", "1:3", 1);
        NDArray<BigInteger> array3 = new BasicBigIntegerNDArray(4, 3).permuteDims(1, 0);
        NDArray<BigInteger> array4 = new BasicBigIntegerNDArray(15).fill(2).reshape(3, 5);
        NDArray<BigInteger> array5 = slice.concatenate(1, array2, array3, array4);
        int start = 0;
        ...
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                assertSpecializedEquals(wrapToBigInteger(1), array5.get(i, j));
        start = end;
        end += array3.shape(1);
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                assertSpecializedEquals(wrapToBigInteger(0), array5.get(i, j));
        start = end;
        end += array4.shape(1);
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                assertSpecializedEquals(wrapToBigInteger(2), array5.get(i, j));
    }

    @Test
    void testEquals() {
        NDArray<BigInteger> slice2 = array.slice(1, new Range(1, 4), Range.ALL);
