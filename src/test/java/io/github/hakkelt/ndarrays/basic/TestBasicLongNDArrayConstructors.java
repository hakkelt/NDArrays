...
class TestBasicLongNDArrayConstructors extends TestBase {

    @Test
    void testDimsConstructor() {
        int[] shape = { 2, 4 };
        NDArray<Long> array = new BasicLongNDArray(shape);
    ...
    void testWrongType3DArrayConstructor() {
        ...
        Exception exception = assertThrows(IllegalArgumentException.class, () -> BasicLongNDArray.of(array));
        assertEquals(String.format(Errors.UNSUPPORTED_TYPE, "String"), exception.getMessage());

        String[] array2 = {"asdf", "", null};
        exception = assertThrows(IllegalArgumentException.class, () -> BasicLongNDArray.of(array2));
        assertEquals(String.format(Errors.UNSUPPORTED_TYPE, "String"), exception.getMessage());

        char[][][] array3 = new char[4][5][3];
        exception = assertThrows(IllegalArgumentException.class, () -> BasicLongNDArray.of(array3));
    ...
    void test1DArrayConstructorsWithByte() {
        ...
        for (int i = 0; i < real.length; i++)
            assertSpecializedEquals(Long.valueOf((long) i), array.get(i));
    }
    ...
    void test2DFloatArrayConstructorsWithBigInteger() {
        ...
        for (int i = 0; i < real.length; i++)
            for (int j = 0; j < real[i].length; j++)
                assertSpecializedEquals(Long.valueOf((long) (i * real.length + j)), array.get(i, j));
    }
    ...
    void test3DFloatArrayConstructorsWithBigDecimal() {
        ...
        for (int i = 0; i < real.length; i++)
            for (int j = 0; j < real[i].length; j++)
                for (int k = 0; k < real[i][j].length; k++)
                    assertSpecializedEquals(Long.valueOf((long) ((i * real.length + j) * real[i].length + k)),
                            array.get(i, j, k));
    }
    ...
    void testWrongCopyFrom1DArrayWithFloat() {
        NDArray<Long> array = new BasicLongNDArray(4, 3, 5);
        ...
        NDArray<Long> array2 = new BasicLongNDArray(5);
        ...
        NDArray<Long> array3 = new BasicLongNDArray(3, 5);
