...
class TestBasicLongNDArrayIndexing extends TestBase {
    ...
    void setup() {
        ...
        array.applyWithLinearIndices((value, index) -> wrapToLong(index.doubleValue()));
        slice = array.slice(1, "1:4", ":");
        reshaped = array.reshape(20, 3);
        pArray = array.permuteDims(0, 2, 1);
        mask = new BasicLongNDArray(array.mapWithLinearIndices((value, index) -> wrapToLong((double) (index % 2))));
        masked = array.mask(mask);
    }

    @Test
    void testGetNegativeLinearIndexingWithArray() {
        assertSpecializedEquals(wrapToLong(60 - 5), array.get(-5));
    }

    @Test
    void testGetNegativeLinearIndexingWithSlice() {
        assertSpecializedEquals(wrapToLong((1 * 5 + 2) * 4 + 1), slice.get(-5));
    }

    @Test
    void testGetNegativeLinearIndexingWithReshaped() {
        assertSpecializedEquals(wrapToLong(60 - 5), reshaped.get(-5));
    }

    @Test
    void testGetNegativeLinearIndexingWithPArray() {
        assertSpecializedEquals(wrapToLong((1 * 5 + 4) * 4 + 3), pArray.get(-5));
    }

    @Test
    void testGetNegativeLinearIndexingWithMasked() {
        assertSpecializedEquals(wrapToLong(61 - 6), masked.get(-3));
    }

    @Test
    void testGetNegativeCartesianIndexingWithArray() {
        assertSpecializedEquals(wrapToLong((2 * 5 + 2) * 4 + 2), array.get(2, -3, -1));
    }

    @Test
    void testGetNegativeCartesianIndexingWithSlice() {
        assertSpecializedEquals(wrapToLong((2 * 5 + 3) * 4 + 1), slice.get(2, -1));
    }

    @Test
    void testGetNegativeCartesianIndexingWithReshaped() {
        assertSpecializedEquals(wrapToLong((2 * 5 + 2) * 4 + 2), reshaped.get(10, -1));
    }

    @Test
    void testGetNegativeCartesianIndexingWithPArray() {
        assertSpecializedEquals(wrapToLong((2 * 5 + 3) * 4 + 2), pArray.get(2, -1, -2));
    }

    @Test
    void testGetNegativeCartesianIndexingWithMasked() {
        assertSpecializedEquals(wrapToLong(61 - 6), masked.get(new int[]{-3}));
    }

    @Test
    void testSetLinearIndexingGetCartesianIndexingWithArrayAndDouble() {
        array.set(1.5, (2 * 5 + 2) * 4 + 0);
        assertSpecializedEquals(wrapToLong(1.5), array.get(0, 2, 2));
    }

    @Test
    void testSetLinearIndexingGetCartesianIndexingWithArrayAndBigInteger() {
        array.set(BigInteger.ONE, (2 * 5 + 2) * 4 + 0);
        assertSpecializedEquals(wrapToLong(1), array.get(0, 2, 2));
    }
    ...
    void testSetLinearIndexingGetCartesianIndexingWithSlice() {
        slice.set(1, 2 * 3 + 2);
        assertSpecializedEquals(wrapToLong(1), slice.get(2, -1));
        assertSpecializedEquals(wrapToLong(1), array.get(1, 3, 2));
    }

    @Test
    void testSetLinearIndexingGetCartesianIndexingWithReshaped() {
        reshaped.set(1, 2 * 20 + 9);
        assertSpecializedEquals(wrapToLong(1), reshaped.get(9, 2));
        assertSpecializedEquals(wrapToLong(1), array.get(1, 2, 2));
    }

    @Test
    void testSetLinearIndexingGetCartesianIndexingWithPArray() {
        pArray.set(1, (4 * 3 + 1) * 4 + 1);
        assertSpecializedEquals(wrapToLong(1), pArray.get(1, 1, -1));
        assertSpecializedEquals(wrapToLong(1), array.get(1, 4, 1));
    }

    @Test
    void testSetLinearIndexingGetCartesianIndexingWithMasked() {
        masked.set(1, -3);
        assertSpecializedEquals(wrapToLong(1), masked.get(new int[]{-3}));
        assertSpecializedEquals(wrapToLong(1), array.get(61 - 6));
    }

    @Test
    void testSetCartesianIndexingGetLinearIndexingWithArrayAndDouble() {
        array.set(1.5, 0, 2, -1);
        assertSpecializedEquals(wrapToLong(1.5), array.get((2 * 5 + 2) * 4 + 0));
    }

    @Test
    void testSetCartesianIndexingGetLinearIndexingWithArrayAndBigInteger() {
        array.set(BigInteger.ONE, 0, 2, -1);
        assertSpecializedEquals(wrapToLong(1), array.get((2 * 5 + 2) * 4 + 0));
    }
    ...
    void testSetCartesianIndexingGetLinearIndexingWithSlice() {
        slice.set(1, 2, -1);
        assertSpecializedEquals(wrapToLong(1), slice.get(2 * 3 + 2));
        assertSpecializedEquals(wrapToLong(1), array.get(1, 3, 2));
    }
    ...
    void testSetCartesianIndexingGetLinearIndexingWithReshaped() {
        reshaped.set(1, 9, 2);
        assertSpecializedEquals(wrapToLong(1), reshaped.get(2 * 20 + 9));
        assertSpecializedEquals(wrapToLong(1), array.get(1, 2, 2));
    }
    ...
    void testSetCartesianIndexingGetLinearIndexingWithPArray() {
        pArray.set(1, 1, 1, -1);
        assertSpecializedEquals(wrapToLong(1), pArray.get((4 * 3 + 1) * 4 + 1));
        assertSpecializedEquals(wrapToLong(1), array.get(1, 4, 1));
    }
    ...
    void testSetCartesianIndexingGetLinearIndexingWithMasked() {
        ...
        assertSpecializedEquals(wrapToLong(1), masked.get(-3));
        assertSpecializedEquals(wrapToLong(1), array.get(61 - 6));
    }
