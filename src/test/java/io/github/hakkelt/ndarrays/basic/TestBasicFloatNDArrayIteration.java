...
class TestBasicFloatNDArrayIteration extends TestBase {
    ...
    void setup() {
        ...
        mask = new BasicFloatNDArray(array.mapWithLinearIndices((value, index) -> wrapToFloat(index % 2)));
    ...
    void testParallelStreamWithArray() {
        Float sum = array.stream().parallel().reduce(wrapToFloat(0), (acc, item) -> add(acc, item));
    ...
    void testParallelStreamWithSlice() {
        Float sum = slice.stream().parallel().reduce(wrapToFloat(0), (acc, item) -> add(acc, item));
        Float acc = wrapToFloat(0);
        for (int i = 0; i < slice.length(); i++)
    ...
    void testParallelStreamWithReshaped() {
        Float sum = reshaped.stream().parallel().reduce(wrapToFloat(0), (acc, item) -> add(acc, item));
        Float acc = wrapToFloat(0);
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testParallelStreamWithPArray() {
        Float sum = pArray.stream().parallel().reduce(wrapToFloat(0), (acc, item) -> add(acc, item));
        Float acc = wrapToFloat(0);
        for (int i = 0; i < pArray.length(); i++)
    ...
    void testParallelStreamWithMasked() {
        Float sum = masked.stream().parallel().reduce(wrapToFloat(0), (acc, item) -> add(acc, item));
        Float acc = wrapToFloat(0);
        for (int i = 0; i < masked.length(); i++)
    ...
    void testCollectorWithArray() {
        NDArray<Float> increased = array.stream()
    ...
    void testCollectorWithReshaped() {
        NDArray<Float> increased = reshaped.stream()
            .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(reshaped.shape()));
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testCollectorWithMasked() {
        NDArray<Float> increased = masked.stream()
            .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(masked.shape()));
        for (int i = 0; i < masked.length(); i++)
    ...
    void testParallelCollectorWithArray() {
        NDArray<Float> increased = array.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(array.shape()));
        for (int i = 0; i < array.length(); i++)
            assertSpecializedEquals(add(array.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithSlice() {
        NDArray<Float> increased = slice.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(slice.shape()));
        for (int i = 0; i < slice.length(); i++)
            assertSpecializedEquals(add(slice.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithReshaped() {
        NDArray<Float> increased = reshaped.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(reshaped.shape()));
        for (int i = 0; i < reshaped.length(); i++)
            assertSpecializedEquals(add(reshaped.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithPArray() {
        NDArray<Float> increased = pArray.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(pArray.shape()));
        for (int i = 0; i < pArray.length(); i++)
            assertSpecializedEquals(add(pArray.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithMasked() {
        NDArray<Float> increased = masked.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicFloatNDArray.getCollector(masked.shape()));
        for (int i = 0; i < masked.length(); i++)
            assertSpecializedEquals(add(masked.get(i), 1), increased.get(i));
    ...
    void testApplyWithReshaped() {
        NDArray<Float> array2 = new BasicFloatNDArray(reshaped);
    ...
    void testMapWithArray() {
        NDArray<Float> array2 = array.map(value -> multiply(value, 2));
        for (int i = 0; i < array.length(); i++)
    ...
    void testMapWithSlice() {
        NDArray<Float> array2 = slice.map(value -> multiply(value, 2));
        for (int i = 0; i < slice.length(); i++)
    ...
    void testMapWithReshaped() {
        NDArray<Float> array2 = reshaped.map(value -> multiply(value, 2));
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testMapWithPArray() {
        NDArray<Float> array2 = pArray.map(value -> multiply(value, 2));
        for (int i = 0; i < pArray.length(); i++)
    ...
    void testMapWithMasked() {
        NDArray<Float> array2 = masked.map(value -> multiply(value, 2));
        for (int i = 0; i < masked.length(); i++)
    ...
    void testMapWithLinearIndicesWithArray() {
        NDArray<Float> array2 = array.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < array.length(); i++)
    ...
    void testMapWithLinearIndicesWithSlice() {
        NDArray<Float> array2 = slice.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < slice.length(); i++)
    ...
    void testMapWithLinearIndicesWithReshaped() {
        NDArray<Float> array2 = reshaped.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testMapWithLinearIndicesWithPArray() {
        NDArray<Float> array2 = pArray.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < pArray.length(); i++)
    ...
    void testMapWithLinearIndicesWithMasked() {
        NDArray<Float> array2 = masked.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < masked.length(); i++)
    ...
    void testForEachWithArray() {
        NDArray<Float> array2 = array.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToFloat(1), value));
    }

    @Test
    void testForEachWithSlice() {
        NDArray<Float> array2 = slice.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToFloat(1), value));
    }

    @Test
    void testForEachWithReshaped() {
        NDArray<Float> array2 = reshaped.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToFloat(1), value));
    }

    @Test
    void testForEachWithPArray() {
        NDArray<Float> array2 = pArray.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToFloat(1), value));
    }

    @Test
    void testForEachWithMasked() {
        NDArray<Float> array2 = masked.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToFloat(1), value));
    }
