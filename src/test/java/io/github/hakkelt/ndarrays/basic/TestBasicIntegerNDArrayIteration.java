...
class TestBasicIntegerNDArrayIteration extends TestBase {
    ...
    void setup() {
        ...
        mask = new BasicIntegerNDArray(array.mapWithLinearIndices((value, index) -> wrapToInteger(index % 2)));
    ...
    void testParallelStreamWithArray() {
        Integer sum = array.stream().parallel().reduce(wrapToInteger(0), (acc, item) -> add(acc, item));
    ...
    void testParallelStreamWithSlice() {
        Integer sum = slice.stream().parallel().reduce(wrapToInteger(0), (acc, item) -> add(acc, item));
        Integer acc = wrapToInteger(0);
        for (int i = 0; i < slice.length(); i++)
    ...
    void testParallelStreamWithReshaped() {
        Integer sum = reshaped.stream().parallel().reduce(wrapToInteger(0), (acc, item) -> add(acc, item));
        Integer acc = wrapToInteger(0);
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testParallelStreamWithPArray() {
        Integer sum = pArray.stream().parallel().reduce(wrapToInteger(0), (acc, item) -> add(acc, item));
        Integer acc = wrapToInteger(0);
        for (int i = 0; i < pArray.length(); i++)
    ...
    void testParallelStreamWithMasked() {
        Integer sum = masked.stream().parallel().reduce(wrapToInteger(0), (acc, item) -> add(acc, item));
        Integer acc = wrapToInteger(0);
        for (int i = 0; i < masked.length(); i++)
    ...
    void testCollectorWithArray() {
        NDArray<Integer> increased = array.stream()
    ...
    void testCollectorWithSlice() {
        NDArray<Integer> increased = slice.stream()
            .map(value -> add(value, 1))
            .collect(BasicIntegerNDArray.getCollector(slice.shape()));
        for (int i = 0; i < slice.length(); i++)
    ...
    void testCollectorWithReshaped() {
        NDArray<Integer> increased = reshaped.stream()
            .map(value -> add(value, 1))
    ...
    void testParallelCollectorWithArray() {
        NDArray<Integer> increased = array.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicIntegerNDArray.getCollector(array.shape()));
        for (int i = 0; i < array.length(); i++)
            assertSpecializedEquals(add(array.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithSlice() {
        NDArray<Integer> increased = slice.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicIntegerNDArray.getCollector(slice.shape()));
        for (int i = 0; i < slice.length(); i++)
            assertSpecializedEquals(add(slice.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithReshaped() {
        NDArray<Integer> increased = reshaped.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicIntegerNDArray.getCollector(reshaped.shape()));
        for (int i = 0; i < reshaped.length(); i++)
            assertSpecializedEquals(add(reshaped.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithPArray() {
        NDArray<Integer> increased = pArray.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicIntegerNDArray.getCollector(pArray.shape()));
        for (int i = 0; i < pArray.length(); i++)
            assertSpecializedEquals(add(pArray.get(i), 1), increased.get(i));
    ...
    void testParallelCollectorWithMasked() {
        NDArray<Integer> increased = masked.stream().parallel()
        .map(value -> add(value, 1))
            .collect(BasicIntegerNDArray.getCollector(masked.shape()));
        for (int i = 0; i < masked.length(); i++)
            assertSpecializedEquals(add(masked.get(i), 1), increased.get(i));
    ...
    void testMapWithArray() {
        NDArray<Integer> array2 = array.map(value -> multiply(value, 2));
        for (int i = 0; i < array.length(); i++)
    ...
    void testMapWithSlice() {
        NDArray<Integer> array2 = slice.map(value -> multiply(value, 2));
        for (int i = 0; i < slice.length(); i++)
    ...
    void testMapWithReshaped() {
        NDArray<Integer> array2 = reshaped.map(value -> multiply(value, 2));
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testMapWithPArray() {
        NDArray<Integer> array2 = pArray.map(value -> multiply(value, 2));
        for (int i = 0; i < pArray.length(); i++)
    ...
    void testMapWithMasked() {
        NDArray<Integer> array2 = masked.map(value -> multiply(value, 2));
        for (int i = 0; i < masked.length(); i++)
    ...
    void testMapWithLinearIndicesWithArray() {
        NDArray<Integer> array2 = array.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < array.length(); i++)
    ...
    void testMapWithLinearIndicesWithSlice() {
        NDArray<Integer> array2 = slice.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < slice.length(); i++)
    ...
    void testMapWithLinearIndicesWithReshaped() {
        NDArray<Integer> array2 = reshaped.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < reshaped.length(); i++)
    ...
    void testMapWithLinearIndicesWithPArray() {
        NDArray<Integer> array2 = pArray.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < pArray.length(); i++)
    ...
    void testMapWithLinearIndicesWithMasked() {
        NDArray<Integer> array2 = masked.mapWithLinearIndices((value, index) -> multiply(value, index));
        for (int i = 0; i < masked.length(); i++)
    ...
    void testMapWithCartesianIndexWithReshaped() {
        NDArray<Integer> array2 = reshaped
                .mapWithCartesianIndices((value, indices) -> multiply(value, indices[0]));
        for (int i = 0; i < array2.length(); i++)
    ...
    void testForEachWithArray() {
        NDArray<Integer> array2 = array.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToInteger(1), value));
    }

    @Test
    void testForEachWithSlice() {
        NDArray<Integer> array2 = slice.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToInteger(1), value));
    }

    @Test
    void testForEachWithReshaped() {
        NDArray<Integer> array2 = reshaped.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToInteger(1), value));
    }

    @Test
    void testForEachWithPArray() {
        NDArray<Integer> array2 = pArray.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToInteger(1), value));
    }

    @Test
    void testForEachWithMasked() {
        NDArray<Integer> array2 = masked.similar().fill(1);
        array2.forEach(value -> assertSpecializedEquals(wrapToInteger(1), value));
    }
