...
class TestBasicBigIntegerNDArrayMisc extends TestBase {
    ...
    void testDTypeWithArray() {
        assertEquals(BigInteger.class, array.dtype());
    }

    @Test
    void testDTypeWithSlice() {
        assertEquals(BigInteger.class, slice.dtype());
    }

    @Test
    void testDTypeWithReshaped() {
        assertEquals(BigInteger.class, reshaped.dtype());
    }

    @Test
    void testDTypeWithPArray() {
        assertEquals(BigInteger.class, pArray.dtype());
    }

    @Test
    void testDTypeWithMasked() {
        assertEquals(BigInteger.class, masked.dtype());
    }

    @Test
    void testWrongToArrayWithArray() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> array.toArray(new BigInteger[0][][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 4, 3), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithSlice() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> slice.toArray(new BigInteger[0][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 3, 2), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithReshaped() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> reshaped.toArray(new BigInteger[0][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 3, 2), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithPArray() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> pArray.toArray(new BigInteger[0][][][]));
        assertEquals(String.format(Errors.TOARRAY_DEPTH_MISMATCH, 4, 3), exception.getMessage());
    }

    @Test
    void testWrongToArrayWithMasked() {
        Exception exception = assertThrows(ArrayStoreException.class, () -> masked.toArray(new BigInteger[0][][]));
    ...
    void testNotEqualsWithArray() {
        ...
        assertNotEquals(array, new BasicBigIntegerNDArray(IntStream.of(array.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(array, new BasicBigIntegerNDArray(array.shape()));
    }
    ...
    void testNotEqualsWithSlice() {
        ...
        assertNotEquals(slice, new BasicBigIntegerNDArray(IntStream.of(slice.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(slice, new BasicBigIntegerNDArray(slice.shape()));
    }
    ...
    void testNotEqualsWithReshaped() {
        ...
        assertNotEquals(reshaped, new BasicBigIntegerNDArray(IntStream.of(reshaped.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(reshaped, new BasicBigIntegerNDArray(reshaped.shape()));
    }
    ...
    void testNotEqualsWithPArray() {
        ...
        assertNotEquals(pArray, new BasicBigIntegerNDArray(IntStream.of(pArray.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(pArray, new BasicBigIntegerNDArray(pArray.shape()));
    }
    ...
    void testNotEqualsWithMasked() {
        ...
        assertNotEquals(masked, new BasicBigIntegerNDArray(IntStream.of(masked.shape()).reduce(1, (acc, value) -> acc * value)));
        assertNotEquals(masked, new BasicBigIntegerNDArray(masked.shape()));
    }
    ...
    void testSliceEqualsWithSameType() {
        NDArray<BigInteger> slice2 = array.slice(1, "1:4", "0:");
        assertEquals(slice, slice2);
        NDArray<BigInteger> slice3 = array.copy().slice(1, "1:4", "0:");
        assertEquals(slice, slice3);
    }

    @Test
    void testReshapedEqualsWithSameType() {
        NDArray<BigInteger> reshaped2 = array.reshape(20, 3);
        assertEquals(reshaped, reshaped2);
        NDArray<BigInteger> reshaped3 = array.copy().reshape(20, 3);
    ...
    void testMaskedEqualsWithSameType() {
        ...
        NDArray<BigInteger> masked3 = array.copy().mask(mask);
        assertEquals(masked, masked3);
        NDArray<BigInteger> masked4 = array.mask(value -> value.intValue() % 2 == 1);
        assertEquals(masked, masked4);
        NDArray<BigInteger> masked5 = array.copy().mask(value -> value.intValue() % 2 == 1);
        assertEquals(masked, masked5);
        NDArray<BigInteger> masked6 = array.maskWithLinearIndices((value, i) -> i.intValue() % 2 == 1);
        assertEquals(masked, masked6);
        NDArray<BigInteger> masked7 = array.copy().maskWithLinearIndices((value, i) -> i.intValue() % 2 == 1);
        assertEquals(masked, masked7);
        NDArray<BigInteger> masked8 = array.maskWithCartesianIndices((value, idx) -> value.intValue() % 2 == 1);
        assertEquals(masked, masked8);
        NDArray<BigInteger> masked9 = array.copy().maskWithCartesianIndices((value, idx) -> value.intValue() % 2 == 1);
    ...
    void testFillDoubleWithArray() {
        ...
        for (BigInteger elem : array)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }

    @Test
    void testFillDoubleWithSlice() {
        slice.fill(3);
        for (BigInteger elem : slice)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }
    ...
    void testFillDoubleWithMasked() {
        masked.fill(3);
        for (BigInteger elem : masked)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }

    @Test
    void testFillTWithArray() {
        array.fill(wrapToBigInteger(3));
        for (BigInteger elem : array)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }
    ...
    void testFillTWithReshaped() {
        reshaped.fill(wrapToBigInteger(3));
        for (BigInteger elem : reshaped)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }
    ...
    void testFillTWithPArray() {
        pArray.fill(wrapToBigInteger(3));
        for (BigInteger elem : pArray)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }
    ...
    void testFillTWithMasked() {
        masked.fill(wrapToBigInteger(3));
        for (BigInteger elem : masked)
            assertSpecializedEquals(wrapToBigInteger(3), elem);
    }
    ...
    void testContaintsWithArray() {
        assertTrue(array.contains(wrapToBigInteger(5)));
    }

    @Test
    void testContaintsWithSlice() {
        assertTrue(slice.contains(wrapToBigInteger(5)));
    }

    @Test
    void testContaintsWithReshaped() {
        assertTrue(reshaped.contains(wrapToBigInteger(5)));
    }

    @Test
    void testContaintsWithPArray() {
        assertTrue(pArray.contains(wrapToBigInteger(5)));
    }

    @Test
    void testContaintsWithMasked() {
        assertTrue(masked.contains(wrapToBigInteger(5)));
    }
