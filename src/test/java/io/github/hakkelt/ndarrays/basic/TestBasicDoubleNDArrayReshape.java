...
class TestBasicDoubleNDArrayReshape extends TestBase {
    ...
    void testNotEqual() {
        ...
        assertNotEquals(reshaped, new BasicDoubleNDArray(10, 2, 3));
        assertNotEquals(reshaped, new BasicDoubleNDArray(6, 10));
    }
    ...
    void testToString() {
        ...
        assertEquals(array.getNamePrefix() + " NDArrayView<Double>(20 × 3)", str);
    }
    ...
    void testContentToString() {
        ...
        for (int i = 0; i < 20; i++)
            if (reshaped.dtype() == Complex.class)
                ...
                sb.append(String.format(lineFormat, wrapToDouble(i), wrapToDouble(i + 20), wrapToDouble(i + 40)));
    ...
    void testConcatenate() {
        NDArray<Double> array2 = new BasicDoubleNDArray(13, 3).fill(1);
        ...
        for (int i = reshaped.shape(0); i < reshaped.shape(0) + array2.shape(0); i++)
            for (int j = 0; j < reshaped.shape(1); j++)
                assertSpecializedEquals(wrapToDouble(1), array3.get(i, j));
    }

    @Test
    void testConcatenateMultiple() {
        NDArray<Double> array2 = reshaped.copy().fill(1).slice(":10", ":");
        NDArray<Double> array3 = new BasicDoubleNDArray(3, 5).permuteDims(1, 0);
        NDArray<Double> array4 = new BasicDoubleNDArray(3, 6).fill(2).reshape(6, 3);
        ...
        for (int i = start; i < end; i++)
            for (int j = 0; j < reshaped.shape(1); j++)
                assertSpecializedEquals(wrapToDouble(1), array5.get(i, j));
        start = end;
        end += array3.shape(0);
        for (int i = start; i < end; i++)
            for (int j = 0; j < reshaped.shape(1); j++)
                assertSpecializedEquals(wrapToDouble(0), array5.get(i, j));
        start = end;
        end += array4.shape(0);
        for (int i = start; i < end; i++)
            for (int j = 0; j < reshaped.shape(1); j++)
                assertSpecializedEquals(wrapToDouble(2), array5.get(i, j));
    }
    ...
    void testReshapeReshaped() {
        NDArray<Double> reshaped2 = reshaped.reshape(4, 15);
