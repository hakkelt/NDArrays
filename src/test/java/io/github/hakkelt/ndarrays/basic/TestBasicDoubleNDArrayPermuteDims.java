...
class TestBasicDoubleNDArrayPermuteDims extends TestBase {
    ...
    void testNotEqual() {
        ...
        assertNotEquals(pArray, new BasicDoubleNDArray(6, 10));
        assertNotEquals(pArray, new BasicDoubleNDArray(4, 5, 3));
    }
    ...
    void testToString() {
        ...
        assertEquals(array.getNamePrefix() + " NDArrayView<Double>(4 × 3 × 5)", str);
    }
    ...
    void testContentToString() {
        ...
        sb.append(array.getNamePrefix() + " NDArrayView<Double>(4 × 3 × 5)" + System.lineSeparator());
        for (int i = 0; i < pArray.shape(2); i++) {
            ...
            for (int j = 0; j < pArray.shape(0); j++) {
                if (pArray.dtype() == Complex.class)
                    ...
                    sb.append(String.format(lineFormat, wrapToDouble(i * 4 + j), wrapToDouble(i * 4 + j + 20), wrapToDouble(i * 4 + j + 40)));
            }
    ...
    void testConcatenate() {
        NDArray<Double> array2 = new BasicDoubleNDArray(2, 3, 5).fill(1);
        NDArray<Double> array3 = pArray.concatenate(0, array2);
        ...
        for (int i = pArray.shape(0); i < pArray.shape(0) + array2.shape(0); i++)
            for (int j = 0; j < pArray.shape(1); j++)
                for (int k = 0; k < pArray.shape(1); k++)
                    assertSpecializedEquals(wrapToDouble(1), array3.get(i, j, k));
    }

    @Test
    void testConcatenateMultiple() {
        NDArray<Double> array2 = pArray.copy().fill(1).slice(":", "1:3", ":");
        NDArray<Double> array3 = new BasicDoubleNDArray(2, 5, 4).permuteDims(2, 0, 1);
        NDArray<Double> array4 = new BasicDoubleNDArray(20).fill(2).reshape(4, 1, 5);
        NDArray<Double> array5 = pArray.concatenate(1, array2, array3, array4);
        int start = 0;
        ...
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                for (int k = 0; k < array2.shape(2); k++)
                    assertSpecializedEquals(wrapToDouble(1), array5.get(i, j, k));
        start = end;
        end += array3.shape(1);
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                for (int k = 0; k < array2.shape(2); k++)
                    assertSpecializedEquals(wrapToDouble(0), array5.get(i, j, k));
        start = end;
        end += array4.shape(1);
        for (int i = 0; i < array2.shape(0); i++)
            for (int j = start; j < end; j++)
                for (int k = 0; k < array2.shape(2); k++)
                    assertSpecializedEquals(wrapToDouble(2), array5.get(i, j, k));
    }
