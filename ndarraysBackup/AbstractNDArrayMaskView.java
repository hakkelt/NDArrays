...
abstract class AbstractNDArrayMaskView<T, T2 extends Number> extends AbstractNDArrayView<T, T2> {
    ...
    protected AbstractNDArrayMaskView(NDArray<T> parent, NDArray<?> mask, boolean isInverse) {
        ...
        this.dataLength = indexMapper.shape();
        ...
        if (parent instanceof AbstractNDArrayMaskView) {
            ...
            if (withLinearIndices)
                ...
                parent.streamCartesianIndices()
                    ...
                    .mapToInt(indices -> IndexingOperations.cartesianIndicesToLinearIndex(indices,
    ...
    protected T getUnchecked(int linearIndex) {
        return parent.getUnchecked(indexMapper.get(IndexingOperations.unwrap(linearIndex, length())));
    }

    @Override
    protected T getUnchecked(int... indices) {
        return parent.getUnchecked(indexMapper.get(IndexingOperations.unwrap(indices[0], length())));
    }

    @Override
    protected void setUnchecked(T value, int linearIndex) {
        parent.setUnchecked(value, indexMapper.get(IndexingOperations.unwrap(linearIndex, length())));
    }

    @Override
    protected void setUnchecked(T value, int... indices) {
        parent.setUnchecked(value, indexMapper.get(IndexingOperations.unwrap(indices[0], length())));
    }

    @Override
    protected String printItem(int linearIndex, String format) {
        IndexingOperations.boundaryCheck(linearIndex, length());
        return parent.printItem(indexMapper.get(IndexingOperations.unwrap(linearIndex, length())), format);
    }
